...
class DependencyParser:
    ...
    def _initialize_patterns(self) -> List[Dict]:
        return [
            {'pattern': ['PREP', 'DET', 'NOUN'], 'head': 2, 'relation': 'prepositional_phrase'},
            {'pattern': ['DET', 'NOUN'], 'head': 1, 'relation': 'determiner'},
            {'pattern': ['VERB', 'PREP', 'NOUN'], 'head': 0, 'relation': 'verbal_complement'},
            {'pattern': ['NOUN', 'PREP', 'NOUN'], 'head': 0, 'relation': 'nominal_complement'},
            {'pattern': ['ADJ', 'NOUN'], 'head': 1, 'relation': 'adjective_modifier'},
            {'pattern': ['NOUN', 'ADJ'], 'head': 0, 'relation': 'adjective_modifier'}
        ]
            ...
            for constituent_type, patterns in self.constituent_types.items():
                for pattern_length in range(len(patterns), 0, -1):
                    if i + pattern_length <= len(tagged_words):
                        candidate = [tagged_words[j][1] for j in range(i, i + pattern_length)]
...
class ClauseSegmenter:
    def __init__(self):
        self.clause_boundaries = {'.', '!', '?', ';', ',', 'e', 'mas', 'ou', 'que', 'porque', 'quando'}
...
class AgreementChecker:
    def __init__(self, profile: Dict):
        ...
        self.gender_enabled = self.agreement_rules.get('gender_agreement', False)
        self.number_enabled = self.agreement_rules.get('number_agreement', False)
    ...
    def check_agreement(self, functions: List[Dict]) -> List[Dict]:
        ...
        for i, func in enumerate(functions):
            if func['function'] == 'NOUN' and func['pos'] == 'NOUN':
                modifiers = [f for f in functions if i in f.get('dependencies', [])]
...
class SyntacticComplexityAnalyzer:
    ...
    def _estimate_clauses(self, functions: List[Dict]) -> int:
        verb_count = sum(1 for f in functions if f['function'] == 'VERB' or f['pos'] == 'VERB')
