...
class TranslationCache:
        ...
        if self.cache_file.exists():
            try:
                with open(self.cache_file, 'r', encoding='utf-8') as f:
                    ...
                    self.context_map = defaultdict(
                        list, data.get('context_map', {}))
            except:
                self.mapping = {}
    ...
    def get_most_frequent_translation(self, source_words: List[str]) -> Optional[str]:
        ...
        if key in self.mapping:
            entry = self.mapping[key]
            variations = entry.get('variations', [entry.get('target')])
            if not variations or len(variations) <= 1:
                return variations[0] if variations else entry.get('target')
    ...
    def consolidate_variations(self, min_frequency: int = 3):
        ...
        for key, entry in self.mapping.items():
            if len(entry['variations']) > 1 and self.frequency[key] >= min_frequency:
                most_frequent = self.get_most_frequent_translation(
                    entry['source'])
                if most_frequent:
        ...
        if self.memory_file.exists():
            try:
                with open(self.memory_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    self.phrase_memory = defaultdict(
                        list, data.get('phrase_memory', {}))
                    self.semantic_clusters = defaultdict(
                        list, data.get('semantic_clusters', {}))
            except:
                pass
